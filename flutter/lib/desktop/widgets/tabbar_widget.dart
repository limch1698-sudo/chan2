...
class _TabState extends State<_Tab> with RestorationMixin {
  ...
  Widget _buildTabContent() {
    ...
    // --- 스피커 아이콘 위젯 추가 시작 ---
    final speakerIcon = Obx(() {
      // stateGlobal에 저장된 오디오 상태를 확인 (1단계에서 추가한 변수)
      // 만약 widget.tabType이 원격 화면(remoteScreen)일 때만 작동하게 함
      bool isPlaying = false;
      if (widget.tabType == DesktopTabType.remoteScreen) {
        isPlaying = stateGlobal.audioPlayingStates[widget.tabInfoKey] ?? false;
      }
      ...
      return Offstage(
        ...
        child: Padding(
          ...
          child: Image.asset(
            'assets/speaker.png', // 준비하신 이미지 파일 경로
            width: 14,
            height: 14,
            fit: BoxFit.contain,
          ),
      ...
      if (widget.tabBuilder == null) {
        return Row(
          ...
          children: [
            icon,
            labelWidget,
            speakerIcon, // 탭 라벨 옆에 스피커 아이콘 배치!
          ],
        );
      } else {
        return widget.tabBuilder!(
          ...
          Row(
            ...
            children: [labelWidget, speakerIcon], // 빌더 사용 시에도 배치!
          ),
